...
object AppObjectFactory {
    fun convertFrom(obj: AppJsonObject, appShared: AppShared): AppObject<AppJsonObject>? {
        val ret: Any
        ret = when (obj) {
            is SharedImage -> AppSharedImage(obj, appShared)
            else -> {
...
class VarDict<in JsonObjectType: AppJsonObject, V: AppObject<JsonObjectType>>(val placeHolderFactory: (String) -> V) : AppVariable {
